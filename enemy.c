#ifndef _ENEMY
#define _ENEMY

void player_enemy_collision_push(u8 n)
{
	if (player.y+15>(enemy[n].y)&&player.y<(enemy[n].y+15)){
		if (player.x+6==enemy[n].x||player.x+5==enemy[n].x)
			player.enemy_collision=COL_ENEMY_RIGHT;
		else if (player.x==enemy[n].x+6||player.x==enemy[n].x+5)
			player.enemy_collision=COL_ENEMY_LEFT;
		else if (player.x==enemy[n].x)
			player.enemy_collision=COL_ENEMY_CENTR;
	}
}

void player_enemy_collision_pull(u8 n)
{
	if (player.y+9>enemy[n].y&&player.y-8<enemy[n].y){
		if (player.x+5>=enemy[n].x&&player.x<enemy[n].x)
			player.enemy_collision=COL_ENEMY_LEFT;
		else if (player.x<=enemy[n].x+5&&player.x>enemy[n].x)
			player.enemy_collision=COL_ENEMY_RIGHT;
		else if (player.x==enemy[n].x)
			player.enemy_collision=COL_ENEMY_CENTR;
	}
}

u16 enemy_collision(u8 n)
{
	static u8 i;
	
	static u8 exl, exr, eyu, eyd;
	
	static u16 collision;
	
	collision=0;
	
	exl=(enemy[n].x+1)/4;
	exr=(enemy[n].x+6)/4;
	
	eyu=(enemy[n].y)/8;
	eyd=(enemy[n].y+15)/8;
	
		
	switch (enemy[n].type) {
		case OWL:
			if (map[eyd][exr]==WALL||exr==39)
				collision^=COL_RIGHT;
			if (map[eyd][exl]==WALL||exl==0)
				collision^=COL_LEFT;
			player_enemy_collision_push(n);
		break;
		
		case BAT:
			if (player.y==enemy[n].y){
				if (enemy[n].x-player.x<=48&&player.x<enemy[n].x){
					collision=COL_RIGHT;
					enemy[n].skip=2;
				}
				else if (player.x-enemy[n].x<=48&&player.x>enemy[n].x){
					collision=COL_LEFT;
					enemy[n].skip=2;
				}
			}
			switch (enemy[n].direct){
				case UP:
					if (map[eyu][exl]==WALL||eyu==0)
						collision=COL_UP;
				break;
				case DOWN:
					if (map[eyd][exl]==WALL||map[eyd][exl]==WATER||map[eyd][exr]==WATER||eyd==21)
						collision=COL_DOWN;
				break;
				case LEFT:
					if (map[eyd][exl-1]==WALL||map[eyu][exl-1]==WALL||exl==0){
						enemy[n].direct=DOWN;
						collision=COL_DOWN;
					}
				break;
				case RIGHT:
					if (map[eyd][exr]==WALL||map[eyu][exr]==WALL||exr==39){
						enemy[n].direct=DOWN;
						collision=COL_DOWN;
					}
				break;
			}
			
			player_enemy_collision_pull(n);
		break;
		
		case ICE_SPIKE:
			if (player.y==enemy[n].y){
				if (enemy[n].x-player.x<=24&&player.x<enemy[n].x){
					collision=COL_RIGHT;
					enemy[n].skip=6;
				}
				else if (player.x-enemy[n].x<=24&&player.x>enemy[n].x){
					collision=COL_LEFT;
					enemy[n].skip=6;
				}
			}
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision=COL_LEFT;
			player_enemy_collision_push(n);
		break;
		
		case SPIDER:
			if (player.y==enemy[n].y){
				if (enemy[n].x-player.x<=60&&player.x<enemy[n].x){
					collision=COL_RIGHT;
					enemy[n].skip=4;
				}
				else if (player.x-enemy[n].x<=60&&player.x>enemy[n].x){
					collision=COL_LEFT;
					enemy[n].skip=4;
				}
			}
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision=COL_LEFT;
			player_enemy_collision_pull(n);
		break;
		
		case ZOMBI:
			switch (enemy[n].direct){
				case WAIT:
					if (player.y==enemy[n].y){
						if (player.x<enemy[n].x&&enemy[n].x-player.x<=100&&map[eyd][exl-1]!=BLOCK){
							collision=COL_RIGHT;
							enemy[n].skip=8;
						}
						if (player.x>enemy[n].x&&player.x-enemy[n].x<=100&&map[eyd][exr]!=BLOCK){
							collision=COL_LEFT;
							enemy[n].skip=8;
						}
					}
				break;
				case RIGHT:
					if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
						collision=COL_RIGHT;
					if (map[eyd][exr]==BLOCK){
						enemy[n].direct=WAIT;
						enemy[n].skip=15;
					}
				break;
				case LEFT:
					if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
						collision=COL_LEFT;
					if (map[eyd][exl-1]==BLOCK){
						enemy[n].direct=WAIT;
						enemy[n].skip=15;
					}
				break;
			}
			player_enemy_collision_pull(n);
		break;
			
		case ZOMBI_HAT:
			switch (enemy[n].direct){
				case WAIT:
					if (player.y==enemy[n].y){
						if (player.x<enemy[n].x&&map[eyd][exl-1]!=WALL&&map[eyd][exl-1]!=BLOCK){
							collision=COL_RIGHT;
							enemy[n].skip=6;
						}
						if (player.x>enemy[n].x&&map[eyd][exr+1]!=WALL&&map[eyd][exr+1]!=BLOCK){
							collision=COL_LEFT;
							enemy[n].skip=6;
						}
					}
				break;
				case RIGHT:
					if (player.y<enemy[n].y-16||player.y>enemy[n].y+16||map[eyd][exr+1]==BLOCK){
						enemy[n].direct=WAIT;
						enemy[n].skip=20;
					}
					if (map[eyd][exr]==WALL||exr==39)
						collision=COL_RIGHT;
					if (map[eyd+1][exl]==EMPTY){
						collision=COL_STALACT;
						enemy[n].direct=STALACT_DOWN;
						enemy[n].skip=10;
					}
				break;
				case LEFT:
					if (player.y<enemy[n].y-16||player.y>enemy[n].y+16||map[eyd][exl-1]==BLOCK){
						enemy[n].direct=WAIT;
						enemy[n].skip=20;
					}
					if (map[eyd][exl]==WALL||exl==0)
						collision=COL_LEFT;
					if (map[eyd+1][exr]==EMPTY){
						collision=COL_STALACT;
						enemy[n].direct=STALACT_DOWN;
						enemy[n].skip=10;
					}
				break;
				case STALACT_DOWN:
					if (map[eyd][exl]==WALL)
						collision=COL_DOWN;
					for (i=1;i<=enemy_summ;i++){
						if (n==i)
							continue;
						if (enemy[i].y<=enemy[n].y)
							if (enemy[i].x+3>=enemy[n].x&&enemy[i].x<=enemy[n].x+3)
							enemy[i].health=0;
						}
				break;
			}
			player_enemy_collision_pull(n);
		break;
		
		case S_SLIME:
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision^=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision^=COL_LEFT;
			player_enemy_collision_pull(n);
		break;
		
		case B_SLIME:
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision^=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision^=COL_LEFT;
			player_enemy_collision_pull(n);
		break;
		
		case STALACT:
			switch (enemy[n].direct){
				case WAIT:
					if (player.y>enemy[n].y){
						if (player.x+4>enemy[n].x&&player.x-4<enemy[n].x)
							collision^=COL_STALACT;
					}
					else if (player.status==ST_DOWN_PUSH){
						if (player.x+3>enemy[n].x&&player.x-3<enemy[n].x)
							collision^=COL_STALACT;
					}
				break;
				
				case STALACT_DOWN:
					if (map[eyd][exl]==WALL)
						collision^=COL_DOWN;
					if (player.y==enemy[n].y){
						if (player.x+2>=enemy[n].x&&player.x<=enemy[n].x+2){
							player.health=0;
							update_hud();
						}
					}
					for (i=1;i<=enemy_summ;i++){
						if (n==i)
							continue;
						if (enemy[i].y==enemy[n].y)
							if (enemy[i].x+3>=enemy[n].x&&enemy[i].x<=enemy[n].x+3)
							enemy[i].health=0;
						}
				break;
			}
		break;
		
		case SNOW_JUMP:
			if (player.y==enemy[n].y){
				enemy[n].skip=4;
				if (player.x<enemy[n].x)
					collision=COL_RIGHT;
				else if (player.x>enemy[n].x)
					collision=COL_LEFT;
			}
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision=COL_LEFT;
			
			if (map[eyd][exr]==BLOCK)
				collision=COL_RIGHT;
			if (map[eyd][exl-1]==BLOCK)
				collision=COL_LEFT;
			
			player_enemy_collision_pull(n);
		break;
		
		case MINE_JUMP:
			if (player.y==enemy[n].y){
				enemy[n].skip=3;
				if (player.x<enemy[n].x)
					collision=COL_RIGHT;
				else if (player.x>enemy[n].x)
					collision=COL_LEFT;
			}
			if (map[eyd][exr]==WALL||map[eyd+1][exr]!=WALL||exr==39)
				collision=COL_RIGHT;
			if (map[eyd][exl]==WALL||map[eyd+1][exl]!=WALL||exl==0)
				collision=COL_LEFT;
			
			if (map[eyd][exr]==BLOCK)
				collision=COL_RIGHT;
			if (map[eyd][exl-1]==BLOCK)
				collision=COL_LEFT;
			
			player_enemy_collision_pull(n);
		break;
		
		case ANGRY_PLANT_R:
			if (player.y+8>enemy[n].y&&player.y-8<enemy[n].y){
				if (player.x+9>enemy[n].x&&player.x-1<enemy[n].x)
					collision=COL_DANGER;
			}
			player_enemy_collision_pull(n);
		break;
		
		case ANGRY_PLANT_L:
			if (player.y+8>enemy[n].y&&player.y-8<enemy[n].y){
				if (player.x-9<enemy[n].x&&player.x+1>enemy[n].x)
					collision=COL_DANGER;
			}
			player_enemy_collision_pull(n);
		break;
		
		case ANGRY_PLANT:
			if (player.y<=enemy[n].y&&player.y>=enemy[n].y-16)
				if (player.x>enemy[n].x-8&&player.x<enemy[n].x+8){
					collision=COL_DANGER;
					enemy[n].skip=5;
				}
			player_enemy_collision_pull(n);
		break;
		
		default: collision=0;
	}
	return collision;
}


void enemy_animation(u8 n)
{
	static u16 n_spr;
	
	switch (enemy[n].type) {
		case B_SLIME:
			n_spr=SPR_B_SLIME;
		break;
		case S_SLIME:
			switch (level) {
				case 0:
				n_spr=SPR_S_SLIME;
				break;
				case 1:
				n_spr=SPR_S_SLIME;
				break;
				case 2:
				n_spr=SPR_S_SLIME_LAB;
				break;
			}
		break;
		case OWL:
			n_spr=SPR_OWL;
		break;
		case BAT:
			n_spr=SPR_BAT;
		break;
		case STALACT:
			n_spr=SPR_STALACT;
		break;
		case ICE_SPIKE:
			n_spr=SPR_ICE_SPIKE;
		break;
		case SPIDER:
			n_spr=SPR_SPIDER;
		break;
		case ZOMBI:
			n_spr=SPR_ZOMBI;
		break;
		case ZOMBI_HAT:
			n_spr=SPR_ZOMBI_HAT;
		break;
		case BLOCK:
			switch (level) {
				case 0:
				n_spr=SPR_BLOCK;
				break;
				case 1:
				n_spr=SPR_COAL;
				break;
				case 2:
				n_spr=SPR_COLBA;
				break;
			}
		break;
		
		case SNOW_JUMP:
			n_spr=SPR_SNOW_JUMP;
		break;
		case MINE_JUMP:
			n_spr=SPR_MINE_JUMP;
		break;
		case ANGRY_PLANT_R:
			n_spr=SPR_ANGRY_PLANT_R;
		break;
		case ANGRY_PLANT_L:
			n_spr=SPR_ANGRY_PLANT_L;
		break;
		case ANGRY_PLANT:
			n_spr=SPR_ANGRY_PLANT;
		break;
	}

	switch (enemy[n].direct) {
		
		case ST_DEATH:
			if (enemy[n].frame==n_spr+11){
				enemy[n].direct=FALSE;
				break;
			}
			if (enemy[n].frame<n_spr+8||enemy[n].frame>n_spr+11){
				enemy[n].frame=n_spr+8;
				enemy[n].skip_count=time();
				if (enemy[n].type!=STALACT&&enemy[n].type!=BLOCK)
					sfx_play(SFX_ENEMY_DEATH,8);
			}
			if (enemy[n].skip_count+DEATH_SPEED<=time()){
				enemy[n].skip_count=time();
				enemy[n].frame++;
			}
		break;
		
		case RIGHT:
			if (enemy[n].frame>(n_spr+3))
				enemy[n].frame=n_spr;
			enemy[n].frame=enemy[n].frame<(n_spr+3)?++enemy[n].frame:(n_spr+0);
		break;
		
		case LEFT:
			if (enemy[n].frame<(n_spr+4))
				enemy[n].frame=n_spr+4;
			enemy[n].frame=enemy[n].frame<(n_spr+7)?++enemy[n].frame:(n_spr+4);
		break;
			
		case UP:
			if (enemy[n].frame<(n_spr+4))
				enemy[n].frame=n_spr+4;
			enemy[n].frame=enemy[n].frame<(n_spr+7)?++enemy[n].frame:(n_spr+4);
		break;
		
		case DOWN:
			if (enemy[n].frame>(n_spr+3))
				enemy[n].frame=n_spr;
			enemy[n].frame=enemy[n].frame<(n_spr+3)?++enemy[n].frame:(n_spr+0);
		break;
		
		case STALACT_DOWN:
			enemy[n].frame=n_spr+8;
		break;
		
		case WAIT:
			if (enemy[n].frame<(n_spr+12))
				enemy[n].frame=n_spr+12;
			enemy[n].frame=enemy[n].frame<(n_spr+19)?++enemy[n].frame:(n_spr+12);
		break;
		
		case ANGRY:
			if (enemy[n].frame>(n_spr+7))
				enemy[n].frame=n_spr;
			enemy[n].frame=enemy[n].frame<(n_spr+7)?++enemy[n].frame:(n_spr+0);
		break;
		
		default:
			break;
	}
}

 
void enemy_logic()
{
	static u16 addr;
	static u16 e_collision;
	static u8 n;
	static u8 j;
	
	for (n=1;n<=enemy_summ;n++){
		
		if (enemy[n].direct==FALSE)
			continue;
		
		e_collision=enemy_collision(n);
		
		if (!enemy[n].health){
			enemy[n].direct=ST_DEATH;
			if (enemy[n].type==BLOCK)
				map[(enemy[n].y+15)/8][(enemy[n].x)/4]=0;
			addr=32768+(40*screen)+(40*level_size*enemy[n].y_start+enemy[n].x_start);
			put_mem(PAGE_MAP,addr,0);
		}
		
		if (enemy[n].direct==WAIT) {	
				if ((enemy[n].skip_count+enemy[n].skip)<=time()){
					enemy[n].skip_count=time();
					enemy_animation(n);
				}
				
				if ((e_collision&COL_RIGHT)==COL_RIGHT){
					enemy[n].direct=LEFT;
					enemy[n].skip_count+=enemy[n].skip;
				}
				if ((e_collision&COL_LEFT)==COL_LEFT){
					enemy[n].direct=RIGHT;
					enemy[n].skip_count+=enemy[n].skip;
				}
				if ((e_collision&COL_STALACT)==COL_STALACT)
					enemy[n].direct=STALACT_DOWN;
				if ((e_collision&COL_DANGER)==COL_DANGER)
					enemy[n].direct=ANGRY;
		}

		
		switch (enemy[n].direct) {
			case UP:
				if ((e_collision&COL_RIGHT)==COL_RIGHT)
					enemy[n].direct=LEFT;
				if ((e_collision&COL_LEFT)==COL_LEFT)
					enemy[n].direct=RIGHT;
				if ((e_collision&COL_UP)!=COL_UP&&enemy[n].direct==UP){
					if ((enemy[n].skip_count+enemy[n].skip)<=time()){
						enemy[n].y-=2;
						enemy[n].skip_count=time();
						enemy_animation(n);
					}
				}
				else enemy[n].direct=DOWN;
			break;
			
			case DOWN:
				if ((e_collision&COL_RIGHT)==COL_RIGHT)
					enemy[n].direct=LEFT;
				if ((e_collision&COL_LEFT)==COL_LEFT)
					enemy[n].direct=RIGHT;
				if ((e_collision&COL_DOWN)!=COL_DOWN&&enemy[n].direct==DOWN){
					if ((enemy[n].skip_count+enemy[n].skip)<=time()){
						enemy[n].y+=2;
						enemy[n].skip_count=time();
						enemy_animation(n);
					}
				}
				else enemy[n].direct=UP;
			break;

			
			case RIGHT:
				if ((e_collision&COL_RIGHT)!=COL_RIGHT){
					if ((enemy[n].skip_count+enemy[n].skip)<=time()){
						enemy[n].x++;
						enemy[n].skip_count=time();
						enemy_animation(n);
					}
				}
				else enemy[n].direct=LEFT;
			break;
			
			case LEFT:
				if ((e_collision&COL_LEFT)!=COL_LEFT){
					if ((enemy[n].skip_count+enemy[n].skip)<=time()){
						enemy[n].x--;
						enemy[n].skip_count=time();
						enemy_animation(n);
					}
				}
				else enemy[n].direct=RIGHT;
			break;
			
			case STALACT_DOWN:
				if ((e_collision&COL_DOWN)!=COL_DOWN){
					enemy[n].skip+=stalact_speed;
					for (j=10;j<enemy[n].skip;j++)
						if ((enemy_collision(n)&COL_DOWN)!=COL_DOWN)
						enemy[n].y++;
					enemy_animation(n);
				}
				else{
					enemy[n].health=0;
					sfx_play(SFX_STALACTIT,8);
				}
			break;
			
			case ANGRY:
				if ((e_collision&COL_DANGER)!=COL_DANGER){
						enemy[n].direct=WAIT;
						enemy[n].skip=10;
				}
				if ((enemy[n].skip_count+enemy[n].skip)<=time()){
					enemy[n].skip_count=time();
					enemy_animation(n);
				}
			break;
			
			case ST_DEATH:
				enemy_animation(n);
			break;
				
			default: break;
		}
	}
}

#endif