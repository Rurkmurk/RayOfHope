...
void menu_control()
{
	static u8 control[]="1 КЕМПСТОН^ ^2 КУРСОР+ПРОБЕЛ^ ^3 ВЫБРАТЬ СВОЕ";
	bright_down();
	clear_screen(0);
	draw_image(0,0,IMG_BACK_MENU);
...
void intro()
{
	...
	static u8 text3[]=" Нужно туда отправиться и все^проверить.^ Из снаряжения имеется биозащитный^костюм и модифицированный^керосиновый плазмотрон.^Бьет он не далеко, но эффективно,^а керосин можно найти на месте.^ ЛУЧ - наша последняя надежда.";
...
void menu_main()
{
	static u8 menu[]="1 ИГРА^ ^2 УПРАВЛЕНИЕ^ ^3 ИНФОРМАЦИЯ";
